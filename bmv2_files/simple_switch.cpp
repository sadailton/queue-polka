...
SimpleSwitch::egress_thread(size_t worker_id) {
  ...
  while (1) {
    ...
    if(TM_buffer_obj.num_of_read_pkts() < TM_buffer_obj.enqueued_packets())
    {
      egress_buffers.pop_back(worker_id, &port, &priority, &packet);
    }

    while(TM_semaphore_obj.is_TM_available(1,0) == false)
    {
    ...
    if(TM_buffer_obj.valid_pop(packet) == 1)
    {
      if (packet == nullptr) break;
      dequeued_pkt = true;
      TM_semaphore_obj.release_TM_key();
