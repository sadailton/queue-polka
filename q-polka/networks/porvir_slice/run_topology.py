...
def topologia(remote_controller):
    ...
    host_vix_ip = "10.0.0.11/24"
    host_vix = rede.addHost("h1", ip=host_vix_ip, mac = host_vix_mac)
    #hosts.append(host_vix)
    
    host_rj_mac = "aa:bb:cc:dd:ee:02"
    host_rj_ip = "10.0.0.12/24"
    host_rj = rede.addHost("h2", ip=host_rj_ip, mac=host_rj_mac)
    #hosts.append(host_rj)

    host_mg_mac = "aa:bb:cc:dd:ee:03"
    host_mg_ip = "10.0.0.13/24"
    host_mg = rede.addHost("h4", ip=host_mg_ip, mac=host_mg_mac)
    #hosts.append(host_mg)

    host_sp_mac = "aa:bb:cc:dd:ee:04"
    host_sp_ip = "10.0.0.14/24"
    host_sp = rede.addHost("h3", ip=host_sp_ip, mac=host_sp_mac)
    ...
    sw_core_rj = rede.addSwitch("s2", netcfg=True, json=json_file_core, thriftport=50002, switch_config=config_core_rj, loglevel=LOG_LEVEL, cls=P4Switch, priority_queue_num=8)
    
    config_core_mg = os.path.join(path, "./sw_config/s3-commands.txt")
    sw_core_mg = rede.addSwitch("s3", netcfg=True, json=json_file_core, thriftport=50003, switch_config=config_core_mg, loglevel=LOG_LEVEL, cls=P4Switch, priority_queue_num=8)
    ...
    sw_edge_vix = rede.addSwitch("e1", netcfg=True, json=json_file_edge, thriftport=50101, switch_config=config, loglevel=LOG_LEVEL, cls=P4Switch)

    config = os.path.join(path, "./sw_config/e2-rj-commands.txt")
    sw_edge_rj = rede.addSwitch("e2", netcfg=True, json=json_file_edge, thriftport=50102, switch_config=config, loglevel=LOG_LEVEL, cls=P4Switch)

    config = os.path.join(path, "./sw_config/e3-mg-commands.txt")
    sw_edge_mg = rede.addSwitch("e3", netcfg=True, json=json_file_edge, thriftport=50103, switch_config=config, loglevel=LOG_LEVEL, cls=P4Switch)

    config = os.path.join(path, "./sw_config/e4-sp-commands.txt")
    sw_edge_sp = rede.addSwitch("e4", netcfg=True, json=json_file_edge, thriftport=50104, switch_config=config, loglevel=LOG_LEVEL, cls=P4Switch)
