...
// these variables will contain the inputs that will be inserted by the user, to be used later. 
	...
	static unsigned int active_queue;
...
// copy the inputs values :: Todo : they should be removed later and just use the inputs directly.
	...
	if(reset_time.get<uint32_t>() == 1)
	{
		...
		shaping = (in_shaping.get<unsigned int>() != 0);
		...
		//Acorda o switch informando que tem pacote a processar
		//sync_cv.notify_one();
	}
...
void level_controller(std::shared_ptr<packet>& level_packet_ptr, unsigned int level_enq, unsigned int level_id)
	...
	void run_core()
	{
		...
		if ((deq == 1)&&(switch_is_ready == 1))
		{
			...
			// O verdadeiro Round Robin: Inspeciona no máximo todas as filas 1 vez por rodada
            for (unsigned int check_count = 0; check_count < 2; check_count++) 
            {
                head_FS = FB[0];
                std::shared_ptr<packet> target_packet = NULL;

                // 1. Busca um pacote na Fila Ativa atual
                while (head_FS != NULL) 
                {
                    if (head_FS->left != NULL && head_FS->left->object->flow_id == active_queue) {
                        target_packet = head_FS->left->object;
                        break;
                    }
                    head_FS = head_FS->bottom;
                }

                // 2. A Fila Ativa tem pacotes?
                if (target_packet != NULL) 
                {
                    // 3. O pacote cabe no saldo atual (Quota)?
                    if (quota_each_queue[active_queue] >= target_packet->levels_ranks[0]) 
                    {
                        // SUCESSO! Tem pacote e tem crédito. Retira o pacote!
                        dequeued_done_right = true;
                        dequeue_FB(deq_packet_ptr, active_queue, FB[0], time_now);
                        
                        if (deq_packet_ptr != NULL) {
                            // Abate o tamanho exato do pacote do saldo
                            new_quota = quota_each_queue[active_queue] - deq_packet_ptr->levels_ranks[0];
                            quota_each_queue[active_queue] = new_quota;
                            
                            // NÃO avança a Fila Ativa. Apenas retorna (o break garante isso).
                            // Na próxima chamada, o switch voltará direto para esta mesma fila!
                            break; 
                        }
                    } 
                    else 
                    {
                        // Tem pacote, mas ACABOU O CRÉDITO! 
                        // Zera o saldo, avança para a próxima fila e recarrega os quantums
                        quota_each_queue[active_queue] = 0;
                        active_queue = (active_queue + 1) % 2; // Gira de 0 pra 1, ou de 1 pra 0
                        quota_each_queue[active_queue] += quantums[active_queue];
                    }
                } 
                else 
                {
                    // A Fila Ativa está VAZIA!
                    // Aborta esta fila, avança o ponteiro e não recarrega (regra do WDRR)
                    quota_each_queue[active_queue] = 0; // Se esvaziou, perde o resto do crédito
                    active_queue = (active_queue + 1) % 2;
                    quota_each_queue[active_queue] += quantums[active_queue];
                }
            } // Fim do For do Round Robin
		}
	}
	...
	void dequeue_FB(std::shared_ptr<hier_scheduler::packet>& deq_packet_ptr, unsigned int flow_id, std::shared_ptr<hier_scheduler::fifo_bank>& head_FB, unsigned int in_time)
	{
		...
		while (cur_ptr_FB != NULL)
		{
			if ((cur_ptr_FB->flow_id == flow_id) && (cur_ptr_FB->left != NULL))
			{
				...
				{
					if(!shaping || cur_ptr_FB->left->object->pred <= in_time)
					{
