		    ...
		    //if(time_now >= 200000)
		    {
			    ...
			    if(dequeued_done_right == false)
			    {
                    ...
                    for (unsigned int i = 0; i < 2 ;i++)
                    {
                        ...
                        {
                            quota_each_queue.erase(quota_each_queue.begin() + i);
                            quota_each_queue.insert(quota_each_queue.begin() + i, current_quota + quantums[i]);
                        }
                ...
                else if (dequeued_done_right == true)
                {
                    ...
                    for (int i = 1; i>=0 ;i--)
                    {
                        BMLOG_DEBUG("Invoked ELBEDIWY testing of starting the dequeue operation i = {}, current_quota = {}, quantums[i] = {}", i, quota_each_queue[i], quantums[i])
                        quota_each_queue.erase(quota_each_queue.begin() + i);
