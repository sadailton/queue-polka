...
/* CÓDIGO ORIGINAL DO SIMPLE_SWITCH*/
...
#include "TM_buffer_WDRR_adailton.h"
//#include "TM_buffer_RL_SP_NWC.h"

//std::shared_ptr<TM_buffer::buffer> TM_buffer::buffer_head = NULL; // added line
// APAGUE: std::shared_ptr<TM_buffer::buffer> TM_buffer::buffer_head = NULL;
// COLOQUE:
std::unordered_map<unsigned int, std::unique_ptr<Packet>> TM_buffer::packet_map;
std::unordered_map<unsigned int, bool> TM_buffer::dropped_pointers;
//bool TM_semaphore::TM_key_available = true;
//std::vector<bool> TM_semaphore::pending_requests = {false, false};

//TM_semaphore TM_semaphore_obj;

std::mutex global_tm_mutex;
...
SimpleSwitch::SimpleSwitch(bool enable_swap, port_t drop_port,
  ...
  : Switch(enable_swap),
    ...
    egress_buffers(nb_egress_threads,
                   10000, EgressThreadMapper(nb_egress_threads),
                   nb_queues_per_port),
    output_buffer(128),
...
SimpleSwitch::transmit_thread() {
  while (1) {
    ...
    //std::this_thread::sleep_for(std::chrono::milliseconds(10));  // Elbediwy added this
...
SimpleSwitch::enqueue(port_t egress_port, std::unique_ptr<Packet> &&packet) {
    ...
    std::cout << "[SimpleSwitch::enqueue] Enqueueing packet on port " << egress_port << std::endl;
...
SimpleSwitch::ingress_thread() {
  ...
  while (1) {
    ...
    /*while(TM_semaphore_obj.is_TM_available(0,1) == false) // elbediwy added this
    {
      std::this_thread::sleep_for(std::chrono::milliseconds(1));  // elbediwy added this
    }*/

    global_tm_mutex.lock(); //adailton added this
    ...
    global_tm_mutex.unlock(); //adailton added this
...
SimpleSwitch::egress_thread(size_t worker_id) {
  PHV *phv;
  TM_buffer TM_buffer_obj; // added line
  bool dequeued_pkt = false; //elbediwy added this
  while (1) 
  {
    std::unique_ptr<Packet> packet;
    size_t port;
    ...
    // 1. A Lógica Inteligente de Repouso (0% CPU e alto Throughput)
    if (TM_buffer_obj.dequeued_packets() == TM_buffer_obj.enqueued_packets()) {
        // O WDRR está completamente VAZIO. Dorme profundamente usando a trava nativa do BMv2.
        egress_buffers.pop_back(worker_id, &port, &priority, &packet);
    } 
    else if (TM_buffer_obj.num_of_read_pkts() < TM_buffer_obj.enqueued_packets()) {
        // Há novos pacotes para serem lidos do BMv2 para o nosso TM_buffer.
        egress_buffers.pop_back(worker_id, &port, &priority, &packet);
    } 
    else {
        // Há pacotes retidos no WDRR pelo Shaping. Micro-pausa (10 microssegundos e não 1 ms!).
        std::this_thread::sleep_for(std::chrono::microseconds(10)); 
    }
    
    global_tm_mutex.lock(); //adailton added this

    dequeued_pkt = false; // elbediwy added thisxte

    if(TM_buffer_obj.valid_pop(packet) == 1) // added line
    {
      if (packet == nullptr) 
      {
        global_tm_mutex.unlock(); //adailton added this
      ...
      dequeued_pkt = true; // elbediwy added this
      //TM_semaphore_obj.release_TM_key(); // elbediwy added this
      global_tm_mutex.unlock(); //adailton added this
    ...
    /*if(dequeued_pkt == false) // elbediwy added this
    {
      TM_semaphore_obj.release_TM_key(); // elbediwy added this
    }*/
  }
