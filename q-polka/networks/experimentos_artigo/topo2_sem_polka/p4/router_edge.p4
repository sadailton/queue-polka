...
control MyIngress(inout headers hdr,
    ...
    // --- NOVA AÇÃO: Reescrever o campo TOS no switch ---
    action set_tos(bit<8> new_tos) {
        hdr.ipv4.diffserv = new_tos;

    }

    // --- NOVA TABELA: Identifica o host e altera o TOS dinamicamente ---
    table override_tos {
        key = {
            hdr.ipv4.diffserv: exact; // Identifica de qual host o fluxo está vindo
        }
        actions = {
            set_tos;
            NoAction;
        ...
        size = 256;
        default_action = NoAction(); // Por padrão, não altera o TOS de ninguém
    }
    ...
    apply {
       // Se o pacote for IPv4 válido e tiver TTL suficiente
        if (hdr.ipv4.isValid() && hdr.ipv4.ttl > 1) {
            ...
            // 1º: O Switch verifica se o controlador mandou alterar o TOS deste IP
            override_tos.apply(); 
