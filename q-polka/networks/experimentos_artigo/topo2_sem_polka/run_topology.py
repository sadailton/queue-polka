...
def topologia(remote_controller):
    ...
    host5_mac = "aa:00:00:00:00:05"
    host5_ip = "10.0.0.5/24"
    host5 = rede.addHost("h5", ip=host5_ip, mac=host5_mac)  

    host6_mac = "aa:00:00:00:00:06"
    host6_ip = "10.0.0.6/24"
    host6 = rede.addHost("h6", ip=host6_ip, mac=host6_mac)

    hosts: list = [host1, host2, host3, host4, host5, host6]

    # Switches P4 core
    path = os.path.dirname(os.path.relpath(__file__))
    json_file_core = os.path.join(path, "./p4/router_core.json")
    info(json_file_core + "\n")
    ...
    json_file_edge = os.path.join(path, "p4/router_edge.json")
    info(json_file_edge + "\n")
    ...
    config_e5 = os.path.join(path, "./sw_config/e5-commands.txt")
    sw_edge5 = rede.addSwitch("e5", netcfg=True, json=json_file_edge, thriftport=52105, switch_config=config_e5, loglevel=LOG_LEVEL, cls=P4Switch, load_module=wdrr_so) 

    config_e6 = os.path.join(path, "./sw_config/e6-commands.txt")
    sw_edge6 = rede.addSwitch("e6", netcfg=True, json=json_file_edge, thriftport=52106, switch_config=config_e6, loglevel=LOG_LEVEL, cls=P4Switch, load_module=wdrr_so)
    
    switches: list = [sw_core1, sw_core2, sw_core3, sw_core4, sw_edge1, sw_edge2, sw_edge3, sw_edge4, sw_edge5, sw_edge6]
    ...
    rede.addLink(host5, sw_edge5)
    rede.addLink(host6, sw_edge6)
    ...
    rede.addLink(sw_core4, sw_edge5)

    rede.addLink(sw_core3, sw_edge6)
    rede.addLink(sw_core4, sw_edge6)
    ...
    h4.cmd("arp -s 10.0.0.5 aa:00:00:00:00:05")
    h4.cmd("arp -s 10.0.0.6 aa:00:00:00:00:06")
    h5.cmd("arp -s 10.0.0.1 aa:00:00:00:00:01")
    h5.cmd("arp -s 10.0.0.2 aa:00:00:00:00:02")
    h5.cmd("arp -s 10.0.0.3 aa:00:00:00:00:03")
    h5.cmd("arp -s 10.0.0.4 aa:00:00:00:00:04")
    h5.cmd("arp -s 10.0.0.6 aa:00:00:00:00:06")
    h6.cmd("arp -s 10.0.0.1 aa:00:00:00:00:01")
    h6.cmd("arp -s 10.0.0.2 aa:00:00:00:00:02")
    h6.cmd("arp -s 10.0.0.3 aa:00:00:00:00:03")
    h6.cmd("arp -s 10.0.0.4 aa:00:00:00:00:04")

    # Configurando os hosts para enviar tráfego
    
    h6.cmd("python3 ./iperf_test.py &")
    sleep(1)
